...
def execute_command(notify, file):
	...
	if notify:
		channel.basic_publish(exchange=BASE_EXCHANGE + '_Notification', routing_key=BASE_ROUTINGKEY + '_Notification', body=data)	
	else:
		...
		channel.basic_publish(exchange=BASE_EXCHANGE + 'QueryRequest', routing_key=BASE_ROUTINGKEY + 'QueryRequest', body=data)
